...
- 🔥 **L** — **Capa de Contexto Empresarial / sistema de indexación empresarial**. Evolucionar la Base Documental hacia conectores seguros de solo lectura, inventario empresarial, extracción de obras activas, clasificación documental y auditoría transversal de la constructora completa. La indexación debe cubrir archivos subidos al agente y fuentes reales de la empresa (drives, SQL, exports CSV/XLSX, ERP/compras/certificaciones), con estados de preparación (`descubierta`/`inventariada`/`clasificada`/`normalizada`/`indexada`/`operativa`/`observada`), perfil vivo por empresa y una lupa contextual que busque por nombre, entidad, evidencia, relación y significado, no solo por filename. Ver `docs/06_enterprise_context_layer.md`.
...
- ✅ 2026-05-18 — Hardening de boundaries: validación Zod de `fetchRemoteMessages` (AI SDK `safeValidateUIMessages`) y respuesta NVIDIA NIM; logs estructurados en 5 sync no-bloqueantes; columna `indexing_status` en `uploaded_files` con badges en `/dashboard/documents` (pending/degraded/failed + tooltip de error real). Smoke test E2E (`npm run smoke:chat`) corre 3 turnos contra DeepSeek y detecta regresiones del ciclo `reasoning_content`.
