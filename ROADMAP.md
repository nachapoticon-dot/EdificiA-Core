...
> Estado consolidado al **2026-05-19**.
...
- 🔥 **L** — **Motor de proactividad**: CRON jobs / workers que corran análisis diario sobre las obras activas. Base 2026-05-16: `runDailyProjectScan()` + `/api/cron/project-proactivity` detectan riesgos de cronograma, HSE, suministros, curva S y documentación stale. Actualización 2026-05-18: los hallazgos vivos se persisten en `operational_findings`; `audit_log_events` queda como evidencia inmutable/resumen append-only. UI: `GET /api/proactivity/findings` + `useProactivityFindings()` + `ProactivityAlertsBanner` leen desde el read model vivo (filtrado opcional por obra activa). Estado 2026-05-19: endpoint y read model listos; no se pudo crear schedule InsForge porque no hay deployment/URL pública registrada (`NEXT_PUBLIC_APP_URL` apunta a localhost). Pendiente externo: publicar una URL estable y registrar el schedule diario.
...
- ✅ 2026-05-19 — Recovery corto post-hardening: hook versionado `.githooks/pre-push` ejecuta `npm run smoke:chat` cuando se empujan cambios al runtime/prompt/router del agente; `/api/documents/[id]/reindex` + CTA en `/dashboard/contexto/fuentes` permiten reintentar indexación de archivos `degraded`/`failed` desde Storage. Schedule de proactividad sigue pendiente hasta tener URL pública real.
